...
error_msg = SoapClient.exceptions
...
try:
    ...
    print("Agent didn't send platform specifications because:" + error_msg)

try:
    r2=str(client.get_value(number=2).resultaat)
    print "System default encoding: ", r2
except:
    print("Agent didn't send encoding specifications because:" + error_msg)

try:
    r3=str(client.get_value(number=3).resultaat)
    print "Resultaat number=3 :", int(r3) # r3 is a number!
except:
    print("Agent didn't send number because:" + error_msg)
...
try:
    ...
    print("Agent didn't send number of processes because:" + error_msg)

try:
    r5=str(client.get_value(number=5).resultaat)
    print "Count services: ", r5.rstrip()
except:
    print("Agent didn't send number of services because:" + error_msg)

try:
    r6 = str(client.get_value(number=6).resultaat)
    print "CPU Usage: ", r6, "%"
except:
    print("Agent didn't send cpu Usage because:" + error_msg)

try:
    r10=float(client.get_value(number=10).resultaat)
    print "Percentage :", r10
except:
    print("Agent didn't send cpu utilization because:" + error_msg)
...
try:
    ...
    print("Agent didn't free hdd space because:" + error_msg)
...
try:
    ...
    print("Agent didn't occupied space because:" + error_msg)
...
try:
    ...
    print("Agent didn't total free hdd space because:" + error_msg)

try:
    r14 = client.get_value(number=14).resultaat
    print r14
except:
    print("Agent didn't report first ip address because:" + error_msg)
